...
import {
  ...
  mapNodesToPath,
  mergeNodes,
  MORE_FILES_NODE_NAME,
  prepareNodesToDisplay,
  SearchForm,
  SearchFormValues,
  SelectNodeCB,
} from '@directory-inspector/file-explorer';
...
const Landing = () => {
  ...
  const updateNode = (nodes: FileNode[], totalCount = 0) => {
    ...
    if (node) {
      const parsedPath = currentPath
        ...
        .split(sep)
        .filter(Boolean);
      const updatedState = mapNodesToPath(
        node,
        parsedPath,
        newNodes,
        totalCount
  ...
  const onSelectNode: SelectNodeCB = (node, parent) => {
    if (node.fileName === MORE_FILES_NODE_NAME) {
      const requestVariables = {
        ...searchFormValues,
        offset: parent?.nodes?.length ? parent?.nodes?.length - 1 : 0,
        path: parent?.path as string,
      };
      setSearchFormValues(requestVariables);
      refetchFiles(requestVariables);
    }
    ...
    <PageContainer>
      <Paper>
        <Box p={2}>
          ...
          <SubtitleText>
            This SPA allows you to navigate through the directory files.
          </SubtitleText>
        </Box>
        <Divider />
